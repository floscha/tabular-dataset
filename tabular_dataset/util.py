...
def encode_categorical_columns(columns_obj, columns):
  for column_name in columns:
    ...
    columns_obj.ds.df[column_name] = encoder.fit_transform(
      columns_obj.ds.df[columns_obj.column_names].values.astype(str).ravel())
...
def one_hot_categorical_columns(columns_obj, columns):
  old_column_names = columns_obj.column_names
  ohe = OneHotEncoder(categories='auto', sparse=False)
  new_column_names = list(ohe.fit(columns_obj.ds.df[old_column_names])
  ...
  columns_obj.column_names = new_column_names
