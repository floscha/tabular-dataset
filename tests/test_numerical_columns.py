...
def test_ranks_with_dense_method():
    """Use another method for ranking to assert that the method parameter works
    as expected."""
    df = pd.DataFrame({'A': [0, 2, 3, 2]})
    tds = TabularDataset(df, numerical_columns=['A'])
    expected_result = np.array([1, 2, 3, 2])

    tds.numerical.ranks(method='dense')
    actual_result = tds.x[:, 1]
